...
function AdminDashboard() {
  ...
  async function toggleAiPass(userId, currentlyOn) {
    try {
      await updateDoc(doc(db, "users", userId), { aiPass: !currentlyOn });
      setUsers((prev) =>
        prev.map((u) => (u.id === userId ? { ...u, aiPass: !currentlyOn } : u)),
      );
    } catch (error) {
  ...
  const stats = {
    ...
    privatePersons: users.filter((u) => u.userType === "jobseeker").length,
    ...
    <div className="dashboard">
      ...
      <main className="dashboard-main">
        {activeTab === "overview" && (
          <div className="dashboard-content">
            ...
            <div className="stats-grid">
              ...
              <div className="stat-card">
                <span className="stat-number">{stats.privatePersons}</span>
                <span className="stat-label">Privatpersoner</span>
              </div>
              <div className="stat-card">
                  ...
                  {users.map((user) => (
                    <tr key={user.id}>
                      ...
                      <td>
                        <button
                          type="button"
                          className={`button small ${user.aiPass ? "primary" : "secondary"}`}
                          onClick={() =>
                            toggleAiPass(user.id, Boolean(user.aiPass))
                          }
                          title="Ubegrenset AI (etter betaling / manuell oppgradering)"
                        >
            ...
            <div className="preview-frame">
              <div className="preview-section">
                ...
                <p className="preview-description">
                  Her ser bedrifter sine publiserte stillinger
                </p>
                ...
                {jobs.length > 0 ? (
                    ...
                    {jobs.slice(0, 3).map((job) => (
                      <div key={job.id} className="job-card">
                        <div className="job-card-header">
                          <h3>{job.title}</h3>
                          <span
                            className={`status-badge ${job.status === "active" ? "green" : "gray"}`}
                          >
                            {job.status === "active" ? "Aktiv" : "Inaktiv"}
                          </span>
                        </div>
                        <div className="job-card-meta">
                          <span>Sted: {job.location}</span>
                  ...
                  <div className="empty-state">
                    ...
                    <p>
                      Bedrifter kan opprette stillinger via "Ny
                      stilling"-knappen
                    </p>
                  </div>
                )}
              ...
              <div className="preview-section">
                <h2>Søkere</h2>
                <p className="preview-description">
                  Bedrifter ser hvem som har søkt på deres stillinger
                </p>
                ...
                {applications.length > 0 ? (
                  <div className="applications-list">
                    {applications.slice(0, 3).map((app) => (
                      <div key={app.id} className="application-card">
                        ...
                        <span
                          className={`status-badge ${app.status === "pending" ? "yellow" : "green"}`}
                        >
                  ...
                  <div className="empty-state">
                    ...
                    <p>
                      Søknader vises her når privatpersoner søker på stillinger
                    </p>
                  </div>
                )}
              </div>
        ...
        {activeTab === "preview-user" && (
          <div className="dashboard-content">
            <div className="preview-header">
              <h1>Privatkonto-dashboard (forhåndsvisning)</h1>
              <p>Slik ser dashboardet ut for privatpersoner</p>
            </div>
            ...
            <div className="preview-frame">
              <div className="preview-section">
                <h2>Mine søknader</h2>
                <p className="preview-description">
                  Privatpersoner ser status på søknadene sine her
                </p>
                ...
                {applications.length > 0 ? (
                  ...
                  <div className="empty-state">
                    ...
                    <p>
                      Når privatpersoner sender søknader, vises de her med
                      status
                    </p>
                  </div>
                )}
              ...
              <div className="preview-section">
                ...
                <p className="preview-description">
                  Privatpersoner ser ledige stillinger de kan søke på
                </p>
